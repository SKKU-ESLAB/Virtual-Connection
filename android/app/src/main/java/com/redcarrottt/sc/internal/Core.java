...
public class Core {
    ...
    public int send(byte[] dataBuffer, int dataLength, boolean isControl) {
        ...
        if (!(packetSize > 0)) {
            Logger.ERR(kTag,
                    "Failed to make protocol data to send(" + currOffset + "/" + dataLength + ")");
        ...
        if (sentBytes < 0) {
            Logger.ERR(kTag,
                    "Sending stopped (" + currOffset + "/" + dataLength + ") by " + sentBytes);
            throw new AssertionError();
        }
    ...
    public boolean isAdaptersSwitching() {
        for (ClientAdapter adapter : this.mAdapters) {
            int adapterState = adapter.getState();
            if (adapterState != ClientAdapter.State.kActive && adapterState != ClientAdapter.State.kDisconnected) {
                return false;
            }
        }
        return true;
    ...
    private static void doneStartCoreTx(Core caller, boolean isSuccess,
                                        OnStartSCResult resultListener) {
        ...
        void start() {
            ...
            synchronized (this.mCaller.mAdapters) {
                for (ClientAdapter adapter : this.mCaller.mAdapters) {
                    int adapterState = adapter.getState();
                    if (adapterState != ClientAdapter.State.kDisconnected && adapterState != ClientAdapter.State.kDisconnecting) {
                        this.mAdaptersCount++;
                    }
                }
            }
