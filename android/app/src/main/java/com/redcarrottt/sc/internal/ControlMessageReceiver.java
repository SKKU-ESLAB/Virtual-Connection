...
public class ControlMessageReceiver {
    ...
    private class ReceivingThread extends Thread {
        ...
        public boolean loopInternal() {
            ...
            switch (controlMessageCode) {
                ...
                case ControlMessageProtocol.CMCode.kRetransmit: {
                    // Retransmit type
                    // Divide the message into second line, third line, fourth line
                    separatorPos = otherLines.indexOf('\n');
                    String secondLine = otherLines.substring(0, separatorPos);
                    ...
                    int segType = Integer.parseInt(secondLine);
                    int seqNoStart = Integer.parseInt(thirdLine);
                    int seqNoEnd = Integer.parseInt(fourthLine);

                    onReceiveRetransmitMessage(segType, seqNoStart, seqNoEnd);
                    break;
                }
                case ControlMessageProtocol.CMCode.kQueueStatus: {
                    // Queue Status type
                    // Divide the message into second line, third line
                    separatorPos = otherLines.indexOf('\n');
                    String secondLine = otherLines.substring(0, separatorPos);
                    String thirdLine = otherLines.substring(separatorPos + 1);

                    int lastSeqNoControl = Integer.parseInt(secondLine);
                    int lastSeqNoData = Integer.parseInt(thirdLine);
                    onReceiveQueueStatusMessage(lastSeqNoControl, lastSeqNoData);
                    break;
                }
                ...
                default: {
                    Logger.ERR(kTag,
                            "Unknown control message code (" + controlMessageCode + ")" + "!\n" + message);
                    break;
                }
            }
    ...
    private void onReceiveDisconnectMessage(int adapterId, int finalSeqNoControl,
                                            int finalSeqNoData) {
    ...
    private void onReceiveRetransmitMessage(int segType, int seqNoStart, int seqNoEnd) {
        Logger.VERB(kTag, "Receive(Control Msg): Request(Retransmit " + segType + " / " + "seq_no"
                + "=" + seqNoStart + "~" + seqNoEnd + ")");
        // TODO: not implemented yet
    }

    private void onReceiveQueueStatusMessage(int lastSeqNoControl, int lastSeqNoData) {
        Logger.VERB(kTag,
                "Receive(Control Msg): Request(Queue Status): lastSeqNoControl=" + lastSeqNoControl + " / lastSeqNoData=" + lastSeqNoData + ")");
        // TODO: not implemented yet
    }
