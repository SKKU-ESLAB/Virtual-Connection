...
IdleTestRunner *IdleTestRunner::sTestRunner = NULL;

void IdleTestRunner::start() {
  assert(this->mThroughput > 0);
  assert(this->mSendWhen == 0 || this->mSendWhen == 1 || this->mSendWhen == 2 ||
         this->mSendWhen == 3);

  printf("** IdleTestRunner Start\n");
  printf(" ** Mode: %d\n", this->mSendWhen);
  ...
  IdleTestRunner::sTestRunner = this;
  sc::start_sc(IdleTestRunner::on_start_sc);
}
...
void IdleTestRunner::on_start_sc(bool is_success) {
  IdleTestRunner *self = IdleTestRunner::sTestRunner;
  ...
  self->mThread =
      new std::thread(std::bind(&IdleTestRunner::receiving_thread, self));
  self->mThread->detach();

  // 1. Send test data to warm-up the initial connection
  printf(" ** Send Test Data\n");
  printf(" ** 15 sec. remains\n");
  self->send_test_data();
  ::sleep(15);
...
void IdleTestRunner::send_test_data() {
...
#define MAX_SEC 5
void IdleTestRunner::send_workload(void) {
  ...
  for (int i = 0; i < max_iter; i++) {
    ...
    // Send data
    IdleTestRunner::generate_simple_string(data_buffer, data_size);
...
void IdleTestRunner::receiving_thread(void) {
...
void IdleTestRunner::onUpdateServerAdapterState(sc::ServerAdapter *adapter,
...
char *IdleTestRunner::generate_random_string(char *str, size_t size) {
...
char *IdleTestRunner::generate_simple_string(char *str, size_t size) {
  memset(str, 'a', size - 1);
  memset(str + size - 1, '\0', 1);
  return str;
}

IdleTestRunner *IdleTestRunner::test_runner(int bt_wfd_mode, int throughput) {
  return new IdleTestRunner(bt_wfd_mode, throughput);
}