...
class IdleTestRunner : sc::ServerAdapterStateListener {
public:
  void start();
  static void on_start_sc(bool is_success);

private:
  static IdleTestRunner *sTestRunner;
...
public:
  static IdleTestRunner *test_runner(int send_when, int throughput);

private:
  IdleTestRunner(int send_when, int throughput) {
