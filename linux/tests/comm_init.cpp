...
int main(void) {
  ...
  snprintf(cmdLine, 500, "%s %s down", HCICONFIG_PATH, DEFAULT_BT_INTERFACE_NAME);
  system(cmdLine);

  // Step 2. Bluetooth ON
  LOG_VERB("Init Step 2. Bluetooth ON");

  snprintf(cmdLine, 500, "%s %s up piscan", HCICONFIG_PATH, DEFAULT_BT_INTERFACE_NAME);
  system(cmdLine);
  ...
  snprintf(cmdLine, 500, "%s %s down", IFCONFIG_PATH, DEFAULT_WFD_INTERFACE_NAME);
  system(cmdLine);
  ...
  snprintf(cmdLine, 500, "%s %s", IFUP_PATH, DEFAULT_WFD_INTERFACE_NAME);
  system(cmdLine);
...
int ping_wpa_cli(char ret[], size_t len) {
  char const *const params[] = {"wpa_cli",  "-i", DEFAULT_WFD_INTERFACE_NAME, "ping", NULL};
