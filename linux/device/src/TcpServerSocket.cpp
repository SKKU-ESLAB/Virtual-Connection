...
bool TcpServerSocket::open_impl(bool is_send_request) {
  ...
  if (setsockopt(this->mServerSocket, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse,
                 sizeof(int)) == -1) {
    LOG_ERR("%s: setsockopt SO_REUSEADDR error", this->get_name());
    return false;
  }
  ...
  if (err < 0) {
    LOG_ERR("%s: bind error", this->get_name());
    return false;
  }

  err = ::listen(this->mServerSocket, 5);
  if (err < 0) {
    LOG_ERR("%s: listen error", this->get_name());
    return false;
  }
  ...
  for (int tries = 0; tries < kMaxTries; tries++) {
    ...
    if (this->mClientSocket >= 0) {
      ...
      if (errno == EINTR) {
        LOG_WARN(
            "open_impl(%s): Accept FAILED (interrupted) Retry to accept...",
            this->get_name());
      } else {
...
int TcpServerSocket::send_impl(const void *data_buffer, size_t data_length) {
  ...
  while (sent_bytes < data_length) {
    ...
    int once_sent_bytes =
        ::send(this->mClientSocket, (void*)buffer_ptr, buffer_length, MSG_NOSIGNAL);
