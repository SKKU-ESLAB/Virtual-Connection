...
public:
  ...
  virtual void prepare_switch(int prev_id, int next_id) = 0;
  virtual void cancel_preparing_switch(int prev_id, int next_id) = 0;

private:
  /* Connect */
  void connect_internal(ConnectCallback callback, bool is_send_request);
  bool __connect_internal(bool is_send_request);
...
private:
  struct timeval mTurnOnDeviceStartTS;
  struct timeval mAllowDiscoverP2PStartTS;
  struct timeval mOpenSocketStartTS;
  struct timeval mOpenSocketEndTS;

  struct timeval mCloseSocketStartTS;
  struct timeval mDisallowDiscoverP2PStartTS;
  struct timeval mTurnOffDeviceStartTS;
  struct timeval mWaitSenderStartTS;
  struct timeval mWaitReceiverStartTS;
  struct timeval mWaitReceiverEndTS;
...
public:
  ...
  ~ServerAdapter() {
    ...
    this->mReceiveDataSize.stop_measure_speed();
  }
