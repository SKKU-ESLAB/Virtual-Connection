 ...
 *
 */
typedef enum { kSLControl = 0, kSLData = 1, kNumSL = 2 } SentSegListType;
...
public:
  ...
  void add_free_segment_to_list(Segment *seg);
  Segment *pop_free_segment(void);
  void deallocate_all_the_free_segments(void);

  /* Sending-failed segment handling */
  void add_failed_segment_to_list(Segment *seg);
  Segment *pop_failed_segment(void);
  void deallocate_sent_segments_by_peer(uint32_t last_seq_no_control,
                                        uint32_t last_seq_no_data);

  /* Retransmission */
  void request_retransmit_missing_segments(void);
  void retransmit_missing_segments_by_peer(int seg_type, uint32_t seq_no_start,
                                           uint32_t seq_no_end);
  void add_sent_segment_to_list(int seg_type, Segment *seg_to_add);
  ...
  int get_failed_sending_queue_data_size() {
    return this->mFailedSegmentListLength.get_value() * SEGMENT_DATA_SIZE;
  }
...
private:
  ...
  std::mutex mFailedSegmentListLock;
  std::mutex mSentSegmentListLocks[kNumSL];
  ...
  std::list<Segment *> mFailedSegmentList;
  std::list<Segment *> mSentSegmentLists[kNumSL];

  /* Statistics */
  Counter mSendRequest;
  Counter mQueueArrivalSpeed; /* to achieve the ema of queue arrival speed */
  Counter mFailedSegmentListLength;
...
private:
  ...
  /* Free segment handling */
  void shrink_free_segment_list(uint32_t threshold);
