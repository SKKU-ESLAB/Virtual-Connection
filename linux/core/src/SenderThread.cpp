...
void SenderThread::sender_loop(void) {
  while (this->is_loop_enabled()) {
    ...
    segment_to_send = sm->pop_failed_segment();
#if defined(VERBOSE_DEQUEUE_SEND_CONTROL) || defined(VERBOSE_DEQUEUE_SEND_DATA)
    bool is_failed_segment = (segment_to_send != NULL);
#endif
    ...
    if (is_control) {
      LOG_DEBUG("%s: Send %s Segment (type=Ctrl, seqno=%d, len=%d, ts=%d.%d)",
                ...
                (is_failed_segment ? "Failed" : "Normal"),
    ...
    // If it is suspended, push the segment to the send-fail queue
    {
      if (this->is_loop_paused()) {
        ...
        sm->add_failed_segment_to_list(segment_to_send);
        continue;
      }
    }
    ...
    if (is_control) {
      sm->add_sent_segment_to_list(kSNControl, segment_to_send);
    } else {
      sm->add_sent_segment_to_list(kSNData, segment_to_send);
    }
  }
}