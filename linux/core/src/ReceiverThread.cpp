...
int ReceiverThread::receive_buffer(void *buf, int len) {
  ...
  if (res == -999) {
    ...
    if (!this->mAdapter->is_disconnecting_on_purpose()) {
      ...
      LOG_VERB("Receiving broken at %s (%d / %d; %s)",
               this->mAdapter->get_name(), errno, res, strerror(errno));
    }
    return RES_BREAK;
  } else {
...
void ReceiverThread::receiver_loop(void) {
  while (this->is_loop_enabled()) {
    ...
    Segment *segment_to_receive = sm->pop_free_segment();
    ...
    // Receive header part
    int res = this->receive_buffer(buf, kSegHeaderSize + kSegSize);
    if (res == RES_CONTINUE)
      continue;
    else if (res == RES_BREAK)
      break;
    ...
    // Insert the segment to control/data receive queue
    if (is_control) {
    ...
    segment_to_receive = sm->pop_free_segment();
