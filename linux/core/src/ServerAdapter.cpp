...
void ServerAdapter::connect(ConnectCallback callback, bool is_send_request) {
  ...
  // Connect
  this->connect_internal(callback, is_send_request);
}
...
void ServerAdapter::connect_internal(ConnectCallback callback,
  ...
  bool res = this->__connect_internal(is_send_request);
...
inline uint64_t tv_to_usec(struct timeval tv) {
  return (uint64_t)tv.tv_sec * 1000 * 1000 + tv.tv_usec;
}

bool ServerAdapter::__connect_internal(bool is_send_request) {
  if (this->mDevice == NULL || this->mP2PServer == NULL ||
      this->mServerSocket == NULL) {
    return false;
  }

  gettimeofday(&this->mTurnOnDeviceStartTS, NULL);
  ...
  gettimeofday(&this->mAllowDiscoverP2PStartTS, NULL);
  ...
  gettimeofday(&this->mOpenSocketStartTS, NULL);
  ...
  if (socketState != ServerSocketState::kOpened &&
    ...
    bool res = this->mServerSocket->open(is_send_request);
  ...
  gettimeofday(&this->mOpenSocketEndTS, NULL);
  uint64_t turnOnDeviceStartUS = tv_to_usec(this->mTurnOnDeviceStartTS);
  uint64_t allowDiscoverP2PStartUS = tv_to_usec(this->mAllowDiscoverP2PStartTS);
  uint64_t openSocketStartUS = tv_to_usec(this->mOpenSocketStartTS);
  uint64_t openSocketEndUS = tv_to_usec(this->mOpenSocketEndTS);
  char eventstr[256];
  snprintf(eventstr, 256, "Connect %s: %llu %llu %llu",
           this->get_name(), (allowDiscoverP2PStartUS - turnOnDeviceStartUS),
           (openSocketStartUS - allowDiscoverP2PStartUS),
           (openSocketEndUS - openSocketStartUS));
  LOG_IMP(eventstr);
...
void ServerAdapter::disconnect_internal(DisconnectCallback callback) {
  ...
  SwitchAdapterTransaction::getOnGoing()->setDisconnectStartTS();
...
bool ServerAdapter::__disconnect_internal(ServerAdapterState oldState) {
  ...
  gettimeofday(&this->mCloseSocketStartTS, NULL);
  ...
  gettimeofday(&this->mDisallowDiscoverP2PStartTS, NULL);
  ...
  gettimeofday(&this->mTurnOffDeviceStartTS, NULL);
  ...
  gettimeofday(&this->mWaitSenderStartTS, NULL);
  ...
  gettimeofday(&this->mWaitReceiverStartTS, NULL);
  ...
  gettimeofday(&this->mWaitReceiverEndTS, NULL);

  uint64_t closeSocketStartTS = tv_to_usec(this->mCloseSocketStartTS);
  uint64_t disallowDiscoverP2PStartTS =
      tv_to_usec(this->mDisallowDiscoverP2PStartTS);
  uint64_t turnOffDeviceStartTS = tv_to_usec(this->mTurnOffDeviceStartTS);
  uint64_t waitSenderStartTS = tv_to_usec(this->mWaitSenderStartTS);
  uint64_t waitReceiverStartTS = tv_to_usec(this->mWaitReceiverStartTS);
  uint64_t waitReceiverEndTS = tv_to_usec(this->mWaitReceiverEndTS);
  char eventstr[256];
  snprintf(eventstr, 256,
           "Disconnect %s: %llu %llu %llu %llu %llu",
           this->get_name(), (disallowDiscoverP2PStartTS - closeSocketStartTS),
           (turnOffDeviceStartTS - disallowDiscoverP2PStartTS),
           (waitSenderStartTS - turnOffDeviceStartTS),
           (waitReceiverStartTS - waitSenderStartTS),
           (waitReceiverEndTS - waitReceiverStartTS));
  LOG_IMP(eventstr);
  EventLogging::print_event(eventstr);
...
int ServerAdapter::send(void *buf, size_t len) {
  ...
  memcpy((void *)((uint8_t *)buf + 20), &net_media_start_ts_sec,
         sizeof(int32_t));
  memcpy((void *)((uint8_t *)buf + 24), &net_media_start_ts_usec,
         sizeof(int32_t));
