...
#ifdef VERBOSE_CONTROL_MESSAGE_SENDER
  LOG_DEBUG("Send(Control Msg): Request(Connect %d)", adapter_id);
#endif
}
...
void ControlMessageSender::send_request_disconnect(
  ...
  this->send_control_message(message);
#ifdef VERBOSE_CONTROL_MESSAGE_SENDER
  LOG_DEBUG("Send(Control Msg): Request(Disconnect %d; "
            "final_seq_no_control=%lu; final_seq_no_data=%lu)",
            adapter_id, final_seq_no_control, final_seq_no_data);
#endif
}

void ControlMessageSender::send_request_retransmit(int adapter_id,
                                                   int segment_type,
  ...
  // Write first line (request code)
  message.append(std::to_string(CMCode::kCMCodeRetransmit));
  message.append("\n");

  // Write second line (segment type)
  message.append(std::to_string(segment_type));
  message.append("\n");

  // Write third line (sequence number start)
  message.append(std::to_string(seq_no_start));
  message.append("\n");

  // Write fourth line (sequence number end)
  message.append(std::to_string(seq_no_end));

  // Send the message
  this->send_control_message(message);
}

void ControlMessageSender::send_request_queue_status(
    uint32_t last_seq_no_control, uint32_t last_seq_no_data) {
  std::string message("");

  // Write first line (request code)
  message.append(std::to_string(CMCode::kCMCodeQueueStatus));
  message.append("\n");

  // Write second line (last sequence number in control queue)
  message.append(std::to_string(last_seq_no_control));
  message.append("\n");

  // Write third line (last sequence number in data queue)
  message.append(std::to_string(last_seq_no_data));
...
#ifdef VERBOSE_CONTROL_MESSAGE_SENDER
  LOG_DEBUG("Send(Control Msg): Request(DisconnectAck %d)", adapter_id);
#endif
}
...
#ifdef VERBOSE_CONTROL_MESSAGE_SENDER
  LOG_DEBUG("Send(Control Msg): Request(Sleep %d)", adapter_id);
#endif
}
...
#ifdef VERBOSE_CONTROL_MESSAGE_SENDER
  LOG_DEBUG("Send(Control Msg): Request(WakeUp %d)", adapter_id);
#endif
}
...
void ControlMessageSender::send_noti_private_data(PrivType priv_type,
  ...
  this->send_control_message(message);
#ifdef VERBOSE_CONTROL_MESSAGE_SENDER
  LOG_DEBUG("Send(Control Msg): Request(Priv %d)\n%s", priv_type,
            priv_data_buffer);
#endif
}