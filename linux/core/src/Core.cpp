...
void Core::start() {
  ...
  // Check UID: it should run by root user
  if (::getuid() != 0) {
    LOG_VERB("start(Core): FAILED - not root user");
...
void Core::prepare_switch(int prev_id, int next_id) {
  for (std::vector<ServerAdapter *>::iterator it = this->mAdapters.begin();
       it != this->mAdapters.end(); it++) {
    ServerAdapter *adapter = *it;
    if (adapter != NULL) {
      adapter->prepare_switch(prev_id, next_id);
    }
  }
}
void Core::cancel_preparing_switch(int prev_id, int next_id) {
  for (std::vector<ServerAdapter *>::iterator it = this->mAdapters.begin();
       it != this->mAdapters.end(); it++) {
    ServerAdapter *adapter = *it;
    if (adapter != NULL) {
      adapter->cancel_preparing_switch(prev_id, next_id);
    }
  }
}
