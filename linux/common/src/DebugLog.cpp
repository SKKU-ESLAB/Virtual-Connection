...
void __log(const char *format, const char *fileName, const char *funcName,
  ...
  printf("\e[%d;%dm%s:%d (%s())  ", bg_color, font_color, fileName, lineNo,
         funcName);
#elif defined(LOG_FILE_NAME)
  printf("\e[%d;%dm%s:%d  ", bg_color, font_color, fileName, lineNo);
#elif defined(LOG_FUNC_NAME)
  printf("\e[%d;%dm(%s())  ", bg_color, font_color, funcName);
#else
  printf("\e[%d;%dm", bg_color, font_color);
#endif
  va_start(ap, lineNo);
  vprintf(format, ap);
  va_end(ap);
...
void __func(const char *format, const char *fileName, const char *funcName,
  ...
  va_list ap;
#if defined(LOG_FILE_NAME) && defined(LOG_FUNC_NAME)
  printf("\e[%dm%s:%d (%s())  ", 32, fileName, lineNo, funcName);
#elif defined(LOG_FILE_NAME)
  printf("\e[%dm%s:%d  ", 32, fileName, lineNo);
#elif defined(LOG_FUNC_NAME)
  printf("\e[%dm(%s())  ", 32, funcName);
#else
  printf("\e[%dm", 32);
#endif
  va_start(ap, lineNo);
